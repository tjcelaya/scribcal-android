package com.tjcelaya.scribcal.appfunctions
...
/**
 * Exposes ScribCal's actions to Gemini and other on-device agents.
 *
 ...
 *
 * Built only when `-Pscribcal.appfunctions=true`, and off by default: the API is alpha, needs
 ...
 */
class ScribCalAppFunctions {

    /**
     * Start timing an event type, for example "start my exercise".
     *
     * Fails if that event type is already being tracked — ScribCal keeps one live event per type.
     *
    ...
    private fun AppFunctionContext.handler(): VoiceActionHandler {
        val app = context.applicationContext as ScribCalApplication
...
/**
 * The result of a ScribCal action.
 */
