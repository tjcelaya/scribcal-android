...
/** The five things any voice surface can ask ScribCal to do. */
 ...
 * `scribcal://action/start?name=…` deep link. Keeping the parsing here — pure Kotlin, no Android
 * types — means both paths are covered by the same unit-tested ladder.
 */
object VoiceIntentParser {

    const val SCHEME = "scribcal"
    ...
    const val ACTION_START = "com.tjcelaya.scribcal.action.VOICE_START"
    const val ACTION_STOP = "com.tjcelaya.scribcal.action.VOICE_STOP"
    const val ACTION_RECORD = "com.tjcelaya.scribcal.action.VOICE_RECORD"
    const val ACTION_EXTEND = "com.tjcelaya.scribcal.action.VOICE_EXTEND"
    const val ACTION_STATUS = "com.tjcelaya.scribcal.action.VOICE_STATUS"
    ...
    /** `scribcal://action/{start|stop|record|extend|status}?name=…` */
    fun fromDeepLink(uri: String?): VoiceRequest? {
        ...
        val verb = when (segments.size) {
            // Tolerate scribcal://start as well as the documented scribcal://action/start.
            1 -> segments[0]
