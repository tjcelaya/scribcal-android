...
class PhotosRepository(
    ...
    companion object {
        ...
        private const val CALWRITE_ALBUM_NAME = "CalWrite Events"
        ...
        private const val KEY_CALWRITE_ALBUM_ID = "calwrite_album_id"
    ...
    private var calwriteAlbumReady = false
    private var calwriteAlbumId: String? = null
    ...
    private suspend fun loadAlbumConfiguration() {
        try {
            ...
            if (albumConfig?.googlePhotosAlbumId != null) {
                calwriteAlbumId = albumConfig.googlePhotosAlbumId
                calwriteAlbumReady = true
    ...
    suspend fun initializePhotos(account: Account): Boolean = withContext(Dispatchers.IO) {
        try {
            ...
            if (hasPermission) {
                ...
                isInitialized = true
                calwriteAlbumReady = true // Assume album will be created when needed
                ...
                currentAccount = account
                isInitialized = true
                calwriteAlbumReady = false // Not ready until consent is granted
    ...
    fun isPhotosReady(): Boolean {
        val basicReadiness = isPhotosInitialized() && calwriteAlbumReady && lastConnectionSuccessful
    ...
    fun isHealthy(): Boolean {
        ...
        if (!calwriteAlbumReady || calwriteAlbumId == null) {
            Log.d(TAG, "Photos not healthy: no album configured (albumReady=$calwriteAlbumReady, albumId=$calwriteAlbumId)")
            return false
        }
    ...
    private suspend fun attemptUpload(
        ...
        try {
            ...
            // Step 3: Add to CalWrite album (required for CalWrite photos)
            val albumId = calwriteAlbumId
            if (albumId == null) {
                Log.e(TAG, "No CalWrite album configured - this is required for photo uploads. User should configure an album in Settings first.")
                return@withContext null
            }

            val success = addToCalWriteAlbum(mediaItemId, token)
            if (!success) {
                Log.e(TAG, "Failed to add photo to CalWrite album - upload failed")
                return@withContext null
            }
            ...
            val requestJson = JSONObject().apply {
                put("newMediaItems", JSONArray().apply {
                    put(JSONObject().apply {
                        put("description", "CalWrite event photo")
    ...
    /**
     * Add media item to CalWrite album
     */
    private suspend fun addToCalWriteAlbum(mediaItemId: String, accessToken: String): Boolean = withContext(Dispatchers.IO) {
        try {
            // Use cached album ID (should be set during connection test)
            val albumId = calwriteAlbumId
            if (albumId == null) {
                Log.w(TAG, "CalWrite album ID not available - connection test may have failed")
            ...
            val responseCode = connection.responseCode
            if (responseCode == 200) {
                Log.d(TAG, "Successfully added photo to CalWrite album")
                true
            } else {
                val errorStream = connection.errorStream?.bufferedReader()?.use { it.readText() }
            ...
            Log.w(TAG, "Failed to add to CalWrite album", e)
            false
        }
    }

    /**
     * Set the selected album for CalWrite photos (replaces auto-creation approach)
     */
    ...
    /**
     * Find or create the CalWrite album using database storage with remote verification
    ...
    private suspend fun findOrCreateCalWriteAlbum(accessToken: String): String? = withContext(Dispatchers.IO) {
        try {
            val albumConfigDao = database.albumConfigDao()
            ...
            if (albumConfig?.googlePhotosAlbumId != null) {
                ...
                if (albumExists) {
                    ...
                    calwriteAlbumId = null
                    calwriteAlbumReady = false
                }
            }

            // No stored album OR stored album was deleted, create a new one
            Log.d(TAG, "Creating new CalWrite album...")
            val newAlbumId = createCalWriteAlbum(accessToken)
            ...
            if (newAlbumId != null) {
                ...
                val config = AlbumConfig(
                    ...
                    googlePhotosAlbumName = CALWRITE_ALBUM_NAME,
                    createdAt = System.currentTimeMillis(),
                    lastVerified = System.currentTimeMillis()
                )
                ...
                // Update in-memory cache
                calwriteAlbumId = newAlbumId
                calwriteAlbumReady = true

                Log.d(TAG, "CalWrite album created/updated successfully with new ID: $newAlbumId")
            } else {
                Log.e(TAG, "Failed to create new CalWrite album")
            }

            return@withContext newAlbumId

        } catch (e: Exception) {
            Log.e(TAG, "Failed to find or create CalWrite album", e)
            null
        }
    }
    ...
    /**
     * Create the CalWrite album with automatic retry on 403/401 errors
     */
    private suspend fun createCalWriteAlbum(accessToken: String): String? = withContext(Dispatchers.IO) {
        // First attempt with provided token
        val firstAttempt = attemptCreateCalWriteAlbum(accessToken)
        if (firstAttempt != null) {
            return@withContext firstAttempt
        }
        ...
        if (refreshed) {
            ...
            if (freshToken.isNotEmpty()) {
                return@withContext attemptCreateCalWriteAlbum(freshToken)
            }
        }
    ...
    /**
     * Single attempt to create CalWrite album without retry logic
     */
    private suspend fun attemptCreateCalWriteAlbum(accessToken: String): String? = withContext(Dispatchers.IO) {
        try {
            Log.d(TAG, "Attempting CalWrite album creation: $CALWRITE_ALBUM_NAME")
            ...
            if (responseCode == 200) {
                ...
                Log.d(TAG, "Created CalWrite album: $albumId")
                albumId
            } else {
                val errorStream = connection.errorStream?.bufferedReader()?.use { it.readText() }
                ...
                when (responseCode) {
                    ...
                    400 -> Log.e(TAG, "ERROR 400: Malformed request. Album title might be invalid: '$CALWRITE_ALBUM_NAME'")
            ...
            Log.e(TAG, "Exception creating CalWrite album", e)
            when (e) {
                ...
                if (token != "") {
                    ...
                    if (selectedAlbum?.googlePhotosAlbumId != null) {
                        ...
                        if (albumExists) {
                            ...
                            calwriteAlbumReady = true
                            calwriteAlbumId = selectedAlbum.googlePhotosAlbumId
     ...
     */
    fun getCurrentAlbumName(): String = CALWRITE_ALBUM_NAME
    ...
    fun getPhotosStatus(): String {
        return when {
            ...
            !calwriteAlbumReady || calwriteAlbumId == null -> "Album not ready"
            lastConnectionTest != null -> {
