...
class DriveRepository(private val context: Context) {
    ...
    companion object {
        ...
        private const val CALWRITE_FOLDER_NAME = "CalWrite"
        private const val CONFIG_FILE_NAME = "calwrite_config.json"
    }

    private var driveService: Drive? = null
    private var calwriteFolderId: String? = null
    ...
    private fun restorePersistedState() {
        try {
            // Restore basic state
            calwriteFolderId = storagePreferences.getDriveFolderId()
            ...
            if (storagePreferences.isDriveInitialized() && accountName != null && calwriteFolderId != null) {
                try {
            ...
            Log.d(TAG, "Drive state restored: initialized=${isDriveInitialized()}, folder=$calwriteFolderId, lastTest=$lastConnectionTest, success=$lastConnectionSuccessful")
        } catch (e: Exception) {
    ...
    suspend fun initializeDrive(account: Account): Boolean = withContext(Dispatchers.IO) {
        try {
            ...
            // Now try to ensure CalWrite folder exists
            ensureCalWriteFolderExists()
     ...
     */
    fun isDriveInitialized(): Boolean = driveService != null && calwriteFolderId != null
    ...
    /**
     * Create or find the CalWrite folder in Google Drive
     */
    private suspend fun ensureCalWriteFolderExists() = withContext(Dispatchers.IO) {
        try {
            val drive = driveService ?: throw IllegalStateException("Drive service not initialized")

            // First, check if CalWrite folder already exists
            val query = "name='$CALWRITE_FOLDER_NAME' and mimeType='application/vnd.google-apps.folder' and trashed=false"
            val result = drive.files().list().setQ(query).execute()

            if (result.files.isNotEmpty()) {
                calwriteFolderId = result.files[0].id
                Log.d(TAG, "Found existing CalWrite folder: $calwriteFolderId")
            } else {
                // Create CalWrite folder
                val folderMetadata = File()
                folderMetadata.name = CALWRITE_FOLDER_NAME
                folderMetadata.mimeType = "application/vnd.google-apps.folder"

                val folder = drive.files().create(folderMetadata).execute()
                calwriteFolderId = folder.id
                Log.d(TAG, "Created CalWrite folder: $calwriteFolderId")
            }
        } catch (e: Exception) {
            Log.e(TAG, "Error ensuring CalWrite folder exists", e)
            throw e
        }
    }

    /**
     * Upload a photo to the CalWrite folder and return a shareable link
     */
    suspend fun uploadPhotoAndGetLink(localFilePath: String, fileName: String): String? = withContext(Dispatchers.IO) {
        try {
            val drive = driveService ?: throw IllegalStateException("Drive service not initialized")
            val folderId = calwriteFolderId ?: throw IllegalStateException("CalWrite folder not initialized")
    ...
    /**
     * Upload (create or overwrite) the CalWrite config JSON in the CalWrite Drive folder.
    ...
    suspend fun uploadConfigFile(jsonContent: String): Boolean = withContext(Dispatchers.IO) {
        try {
            ...
            if (calwriteFolderId == null) {
                ensureCalWriteFolderExists()
            }
            val folderId = calwriteFolderId ?: return@withContext false
    ...
    /**
     * Download the CalWrite config JSON from the CalWrite Drive folder, or null if absent.
     */
    suspend fun downloadConfigFile(): String? = withContext(Dispatchers.IO) {
        try {
            ...
            val folderId = calwriteFolderId ?: return@withContext null
    ...
    suspend fun testDriveConnection(): DriveConnectionResult = withContext(Dispatchers.IO) {
        return@withContext try {
            ...
            val folderId = calwriteFolderId ?: return@withContext DriveConnectionResult(
                isConnected = false,
                status = "CalWrite folder not found",
                lastTestTime = null
            )
            ...
            val testContent = "CalWrite Drive connection test\nTimestamp: ${java.util.Date(timestamp)}"
            ...
            val tempFile = kotlin.io.path.createTempFile("calwrite_test", ".txt").toFile()
     ...
     */
    fun getCurrentFolderName(): String = CALWRITE_FOLDER_NAME
    ...
    fun getDriveStatus(): String {
        return when {
            ...
            calwriteFolderId == null -> "Setup required"
