...
/**
 * Serializes CalWrite "configuration" (event types, scheduled events, album config, app
 * settings and the selected calendar) to a single JSON file and exports/imports it to:
 *  - the user's shared storage at Documents/CalWrite/calwrite_config.json (via MediaStore), and
 *  - the CalWrite folder in the user's Google Drive.
 *
...
class ConfigBackupManager(
    ...
    companion object {
        ...
        const val FILE_NAME = "calwrite_config.json"
        // MediaStore RELATIVE_PATH is canonically stored with a trailing slash.
        private val RELATIVE_PATH = Environment.DIRECTORY_DOCUMENTS + "/CalWrite/"
        const val DISPLAY_PATH = "Documents/CalWrite/$FILE_NAME"
    }
    ...
    suspend fun exportToDrive(): BackupResult = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            if (driveRepository.uploadConfigFile(json)) {
                BackupResult.Success("Exported to Google Drive (CalWrite/$FILE_NAME)")
            } else {
    ...
    // === MediaStore (Documents/CalWrite/calwrite_config.json) ===
