...
interface EventDao {
    ...
    /** Local retention: drop completed events older than the cutoff. Google Calendar keeps the durable copy. */
    @Query("DELETE FROM events WHERE endTime IS NOT NULL AND endTime < :cutoff")
    suspend fun deleteCompletedEventsOlderThan(cutoff: Long)
