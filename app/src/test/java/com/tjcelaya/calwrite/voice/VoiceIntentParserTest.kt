...
class VoiceIntentParserTest {
    ...
    fun `parses each documented verb`() {
        ...
        verbs.forEach { (verb, expected) ->
            assertEquals(
                ...
                VoiceIntentParser.fromDeepLink("calwrite://action/$verb")
            )
        }
    }
    ...
    fun `parses the name parameter`() {
        assertEquals(
            ...
            VoiceIntentParser.fromDeepLink("calwrite://action/start?name=Exercise")
        )
    }
    ...
    fun `decodes percent and plus encoded names`() {
        assertEquals(
            "Morning Run",
            VoiceIntentParser.fromDeepLink("calwrite://action/start?name=Morning%20Run")?.typeQuery
        )
        assertEquals(
            "Morning Run",
            VoiceIntentParser.fromDeepLink("calwrite://action/start?name=Morning+Run")?.typeQuery
        )
        assertEquals(
            "Café",
    ...
    fun `ignores unrelated query parameters and fragments`() {
        assertEquals(
            ...
            VoiceIntentParser.fromDeepLink("calwrite://action/stop?source=tasker&name=Exercise#x")
        )
    }

    @Test
    fun `treats a blank name as absent`() {
        assertNull(VoiceIntentParser.fromDeepLink("calwrite://action/stop?name=")?.typeQuery)
        assertNull(VoiceIntentParser.fromDeepLink("calwrite://action/stop?name=%20")?.typeQuery)
    }
    ...
    fun `tolerates casing and a trailing slash`() {
        assertEquals(
            ...
            VoiceIntentParser.fromDeepLink("CALWRITE://Action/Status/")
        )
    }
    ...
    fun `accepts the host-less short form`() {
        assertEquals(
            VoiceRequest(VoiceActionType.START, "Exercise"),
            VoiceIntentParser.fromDeepLink("calwrite://start?name=Exercise")
        )
    }
    ...
    fun `rejects anything that is not a voice deep link`() {
        ...
        assertNull(VoiceIntentParser.fromDeepLink("https://calwrite.app/action/start"))
        assertNull(VoiceIntentParser.fromDeepLink("calwrite://action/pause"))
        assertNull(VoiceIntentParser.fromDeepLink("calwrite://other/start"))
        assertNull(VoiceIntentParser.fromDeepLink("calwrite://a/b/c/start"))
        assertNull(VoiceIntentParser.fromDeepLink("calwrite://"))
    }
